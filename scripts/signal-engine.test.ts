...
import { alertDedupe, alertDeliveries, alertDeliveryAttempts, alertDeliverySlaWindows, alertDestinationStates, alertDlq, alertEndpointHealth, alertNoiseBudgetBreaches, alertNoiseBudgets, alertQualityGateBreaches, alertRuns, alertSlaThresholds, alertSubscriptions, apiKeys, auditEvents, auditExports, incidentEscalationPolicies, incidentEscalations, incidents, portDailyBaselines, ports, signals, tenantSettings, tenantUsers } from "@shared/schema";
...
import { runIncidentEscalations } from "../server/services/incidentEscalationService";
...
async function main() {
  ...
  const withTransientRetry = async <T>(fn: () => Promise<T>, label: string, retries = 2) => {
  ...
  const thresholdTenant = randomUUID();
  const thresholdUser = randomUUID();
  const noiseTenant = randomUUID();
  const noiseUser = randomUUID();
  const qualityTenant = randomUUID();
  const qualityUser = randomUUID();
  const qualityAllowTenant = randomUUID();
  const qualityAllowUser = randomUUID();
  const bundleTenant = randomUUID();
  const bundleUser = randomUUID();
  const bundleDedupeTenant = randomUUID();
  ...
  try {
    ...
    log("baseline + signals seed complete");
    ...
    const auditTenant = randomUUID();
    const auditTenantOther = randomUUID();
    ...
    const destinationAuditTenant = randomUUID();
    ...
    const destinationAuditWriteTenant = randomUUID();
    ...
    const retentionTenant = randomUUID();
    ...
    const purgeTenant = randomUUID();
    const purgeOtherTenant = randomUUID();
    ...
    const rateTenantA = randomUUID();
    const rateTenantB = randomUUID();
    ...
    const tenantIsolated = randomUUID();
    ...
    try {
      ...
      assert(lockResult.rows?.[0]?.locked, "advisory lock acquired for backfill test");
    ...
    const slaSystemTenant = randomUUID();
    const slaSystemUser = randomUUID();
    const [systemMember] = await db.insert(tenantUsers).values({
    ...
    const endpointTenant = randomUUID();
    ...
    const endpointTenantB = randomUUID();
    ...
    const incidentListB = await listIncidents({ tenantId: endpointTenantB, limit: 10 });
    assert.strictEqual(incidentListB.items.length, 0, "tenant isolation for incidents list");

    const incidentCrossTenant = await getIncidentById(endpointTenantB, slaIncident.id);
    assert.strictEqual(incidentCrossTenant, null, "incident detail tenant isolation");

    const metricsTenant = randomUUID();
    ...
    const metricsTenantB = randomUUID();
    await db.insert(incidents).values({
    ...
    const destinationStateTenant = randomUUID();
    ...
    const destFilterTenant = randomUUID();
    ...
    const autoPauseTenant = randomUUID();
    ...
    const destinationsTenant = randomUUID();
    ...
    const endpointAlertTenant = randomUUID();
    ...
    const endpointGateTenant = randomUUID();
    ...
    const automationTenant = randomUUID();
    ...
    const escalationTenant = randomUUID();
    const escalationUser = randomUUID();
    const escalationNow = new Date("2026-02-10T12:00:00Z");
    const escalationOpened = new Date(escalationNow.getTime() - 31 * 60 * 1000);

    await db.insert(tenantUsers).values({
      tenantId: escalationTenant,
      userId: escalationUser,
      email: "escalation@veriscope.dev",
      role: "OWNER",
      status: "ACTIVE",
      createdBy: escalationUser,
    });

    const [escalationSub] = await db.insert(alertSubscriptions).values({
      tenantId: escalationTenant,
      userId: escalationUser,
      entityType: "port",
      entityId: portA,
      ...
      isEnabled: true,
      updatedAt: escalationNow,
    }).returning();

    const [escalationIncident] = await db.insert(incidents).values({
      tenantId: escalationTenant,
      type: "SLA_AT_RISK",
    ...
    await db.insert(incidentEscalationPolicies).values({
      ...
      targetRef: "OWNER",
      enabled: true,
      createdAt: escalationNow,
      updatedAt: escalationNow,
    });

    await runIncidentEscalations({ tenantId: escalationTenant, now: escalationNow });
    const [escalationState] = await db
      .select()
      .from(incidentEscalations)
      .where(eq(incidentEscalations.incidentId, escalationIncident.id))
      .limit(1);
    assert.strictEqual(escalationState?.currentLevel, 1, "escalation level advanced to 1");

    const escalationDeliveries = await db
      .select()
      .from(alertDeliveries)
      .where(and(eq(alertDeliveries.tenantId, escalationTenant), sql`${alertDeliveries.clusterId} like ${`incident-escalation:${escalationTenant}:${escalationIncident.id}%`}`));
    assert.strictEqual(escalationDeliveries.length, 1, "escalation delivery emitted");

    const escalationAudit = await db
      .select()
      .from(auditEvents)
      .where(and(eq(auditEvents.tenantId, escalationTenant), eq(auditEvents.action, "INCIDENT.ESCALATED")));
    assert.strictEqual(escalationAudit.length, 1, "escalation audit emitted");

    await runIncidentEscalations({ tenantId: escalationTenant, now: escalationNow });
    const escalationDeliveriesAgain = await db
      .select()
      .from(alertDeliveries)
      .where(and(eq(alertDeliveries.tenantId, escalationTenant), sql`${alertDeliveries.clusterId} like ${`incident-escalation:${escalationTenant}:${escalationIncident.id}%`}`));
    assert.strictEqual(escalationDeliveriesAgain.length, 1, "escalation idempotent at level 1");
    ...
    await runIncidentEscalations({ tenantId: escalationTenant, now: escalationNow });
    const escalationDeliveriesLevel2 = await db
      .select()
      .from(alertDeliveries)
      .where(and(eq(alertDeliveries.tenantId, escalationTenant), sql`${alertDeliveries.clusterId} like ${`incident-escalation:${escalationTenant}:${escalationIncident.id}%`}`));
    assert.strictEqual(escalationDeliveriesLevel2.length, 2, "escalation advanced to level 2");

    process.env.RATE_LIMIT_GLOBAL_PER_MIN = prevGlobalLimit;
    process.env.RATE_LIMIT_EXPORT_PER_MIN = prevExportLimit;
    process.env.RATE_LIMIT_WRITE_PER_MIN = prevWriteLimit;
    ...
    try {
      ...
      await db.delete(alertSubscriptions).where(eq(alertSubscriptions.userId, qualityUser));
      await db.delete(alertSubscriptions).where(eq(alertSubscriptions.userId, qualityAllowUser));
      await db.delete(alertSubscriptions).where(eq(alertSubscriptions.userId, bundleUser));
      await db.delete(alertSubscriptions).where(eq(alertSubscriptions.userId, bundleDedupeUser));
